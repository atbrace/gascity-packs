...
- a mentioned/targeted delivery's envelope carries an extra `untrusted_context_json` field (local fork only) alongside `untrusted_body_json`: a JSON object with `channel_name` and up to the last 10 messages before the trigger (oldest first, each truncated to 300 characters), plus the forum-post starter message first when the channel is a thread, so `@bot ^^` after an earlier unanswered question has something to look back at; it is fetched only right before delivery for a mentioned message (never on ambient/untargeted paths), a REST failure leaves `recent_messages` empty rather than blocking delivery, and the whole object is capped at ~4KB by dropping the oldest messages first
- a binding's `dispatch_authors` (local fork only; `gc discord bind --dispatch-author <discord_user_id>`, repeatable, plus a required `--dispatch-workdir <path>`) names Discord user ids trusted to have their mentions filed and dispatched as work instead of nudging a session: a mention from one of those ids skips the liaison entirely — the gateway shells out to `bd create` (title = the first line of the message with any seam prefix stripped, ≤80 chars, falling back to `Discord request from <display>` when the body is empty; body = the same envelope text used for a normal delivery, untouched by the prefix strip, tagged `discord-request` and `seam:<seam>`) in `dispatch_workdir` with `BEADS_DIR`/`BEADS_DB` stripped from its environment so `bd` resolves that checkout's own store, then routes the bead through one of three hardcoded seams picked by a leading word in the message (case-insensitive, followed by `:` or a single whitespace char — `fixing this` does not match `fix`): `fix` → `./bin/tools dispatch <bead>`; `recon` → `gc sling sysadmin/homeops.homeops-luna <bead> --on homeops-recon --nudge`; `ops` → `gc sling sysadmin/homeops.homeops-mechanic <bead> --nudge`; no recognized prefix defaults to `recon`. It then posts one ack reply (`filed <id> → <seam>`, or `filed <id>; <seam> dispatch failed: ...`, or `could not file a bead: ...` if even that failed); a mention from any other author on the same binding is unaffected and still goes through the normal session/liaison path
